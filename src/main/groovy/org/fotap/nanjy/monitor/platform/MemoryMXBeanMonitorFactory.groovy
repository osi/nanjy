        ...
        return {
            run: {
                ...
                samples.publish new Sample(
                    name + "/gauge-objects_pending_finalization", now, bean.objectPendingFinalizationCount)
                ...
                ["heap": bean.heapMemoryUsage, "non_heap": bean.nonHeapMemoryUsage].each {
                    ...
                    samples.publish new Sample(
                        name + "/jvm_memory-" + type + "", now, usage.init, usage.used, usage.committed, usage.max);
                }
            }
